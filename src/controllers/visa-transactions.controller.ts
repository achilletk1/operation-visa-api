...
export const visaTransactionsController = {
    ...
    init: (app: any): void => {
        ...
        app.post('/visa-transactions/import/verify', async (req: Request, res: Response) => {
            const data = await visaTransactionsService.verifyTransactionFiles(req.body);
        ...
        app.put('/visa-transactions/import/confirm/:id', async (req: Request, res: Response) => {
            const data = await visaTransactionsService.confirmTransactionFiles(req.params.id);
            ...
            res.status(200).json(data);
            const traitment = await visaTransactionsService.startTraitment(req.params.id);

            if (traitment instanceof Error) {
                await visaTransactionsService.sendPostTransactionFileThreadError(req.params.id);
            }

        });

        app.put('/visa-transactions/import/restart/:id', async (req: Request, res: Response) => {
            const data = await visaTransactionsService.restartConfirmTransactionFiles(req.params.id);
            ...
            res.status(200).json(data);
            const traitment = await visaTransactionsService.restartTraitment(req.params.id);
            if (traitment instanceof Error) {
                await visaTransactionsService.sendPostTransactionFileThreadError(req.params.id);
            }
        ...
        app.delete('/visa-transactions/import/abort/:id', async (req: Request, res: Response) => {
            const data = await visaTransactionsService.abortTransactionFiles(req.params.id);
        ...
        app.get('/visa-transactions/import/array/:id', async (req: Request, res: Response) => {
            const data = await visaTransactionsService.getTransactionFilesDataArray(req.params.id);
