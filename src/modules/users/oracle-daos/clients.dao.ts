...
export const clientsDAO = {
    getClientDatasByNcp: async (ncp: any, age?: string, clc?: string) => {
        ...
        try {
            ...
            from bank.bkcli a
            left join (SELECT a.cli, a.ncp, a.age, a.clc, a.inti, a.cha FROM bank.bkcom a WHERE a.ncp = '${ncp}' and rownum = 1) b on a.cli = b.cli
            left join (SELECT a.cli, a.num FROM bank.bktelcli a WHERE rownum = 1) t1 on b.cli = t1.cli
            left join (SELECT a.cli, a.email FROM bank.bkemacli a WHERE rownum = 1) t2 on b.cli = t2.cli
            where a.cli = b.cli`;
            if (age && clc) {
                query =
                    `
            select
                ...
                a.tid IDTYPE, a.nid IDNUM, a.did date_delivrance_PI, a.vid date_validite_PI, a.lid lieu_delivrance_PI, a.oid organisme_delivrance_PI
            from bank.bkcli a
            left join (SELECT a.cli, a.ncp, a.age, a.clc, a.inti , a.cha FROM bank.bkcom a WHERE a.ncp = '${ncp}' and a.age = '${age}' and a.clc = '${clc}' and rownum = 1) b on a.cli = b.cli
            left join (SELECT a.cli, a.num FROM bank.bktelcli a WHERE rownum = 1) t1 on b.cli = t1.cli
            left join (SELECT a.cli, a.email FROM bank.bkemacli a WHERE rownum = 1) t2 on b.cli = t2.cli
            where a.cli = b.cli`;
            }
