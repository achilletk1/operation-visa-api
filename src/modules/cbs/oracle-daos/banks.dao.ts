...
export const banksDAO = {
    ...
    getBankList: async (): Promise<CbsBankBranch[]> => {
        ...
        try {
            ...
            const query = `
            select
                distinct pays, lib_pays, code_banque, nom_banque, code_agence, (select b.nom from infoc.bkbqe b where b.etab = a.code_banque and b.guib = a.code_agence and rownum = 1) nom_agence
            from
                (
                    select
                        a.pays, a.etap code_banque, a.guip code_agence, (select lib1 from infoc.bknom where ctab = '033' and cacc = a.pays ) lib_pays,
                        (select lib1 from infoc.bknom where ctab = '840' and age = '99500' and cacc = a.etap) nom_banque
                    from infoc.bkbqe a
