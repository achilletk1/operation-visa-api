/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { CategorieWhereUniqueInput } from "./CategorieWhereUniqueInput";
import { ValidateNested } from "class-validator";
import { Type } from "class-transformer";
import { CategorieUpdateInput } from "./CategorieUpdateInput";

@ArgsType()
class UpdateCategorieArgs {
  @ApiProperty({
    required: true,
    type: () => CategorieWhereUniqueInput,
  })
  @ValidateNested()
  @Type(() => CategorieWhereUniqueInput)
  @Field(() => CategorieWhereUniqueInput, { nullable: false })
  where!: CategorieWhereUniqueInput;

  @ApiProperty({
    required: true,
    type: () => CategorieUpdateInput,
  })
  @ValidateNested()
  @Type(() => CategorieUpdateInput)
  @Field(() => CategorieUpdateInput, { nullable: false })
  data!: CategorieUpdateInput;
}

export { UpdateCategorieArgs as UpdateCategorieArgs };
