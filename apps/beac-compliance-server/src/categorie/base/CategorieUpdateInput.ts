/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { InputType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import {
  IsString,
  MaxLength,
  IsOptional,
  ValidateNested,
} from "class-validator";
import { OperationUpdateManyWithoutCategoriesInput } from "./OperationUpdateManyWithoutCategoriesInput";
import { Type } from "class-transformer";
import { PieceJointeUpdateManyWithoutCategoriesInput } from "./PieceJointeUpdateManyWithoutCategoriesInput";

@InputType()
class CategorieUpdateInput {
  @ApiProperty({
    required: false,
    type: String,
  })
  @IsString()
  @MaxLength(1000)
  @IsOptional()
  @Field(() => String, {
    nullable: true,
  })
  description?: string | null;

  @ApiProperty({
    required: false,
    type: String,
  })
  @IsString()
  @MaxLength(1000)
  @IsOptional()
  @Field(() => String, {
    nullable: true,
  })
  nom?: string | null;

  @ApiProperty({
    required: false,
    type: () => OperationUpdateManyWithoutCategoriesInput,
  })
  @ValidateNested()
  @Type(() => OperationUpdateManyWithoutCategoriesInput)
  @IsOptional()
  @Field(() => OperationUpdateManyWithoutCategoriesInput, {
    nullable: true,
  })
  operations?: OperationUpdateManyWithoutCategoriesInput;

  @ApiProperty({
    required: false,
    type: () => PieceJointeUpdateManyWithoutCategoriesInput,
  })
  @ValidateNested()
  @Type(() => PieceJointeUpdateManyWithoutCategoriesInput)
  @IsOptional()
  @Field(() => PieceJointeUpdateManyWithoutCategoriesInput, {
    nullable: true,
  })
  pieceJointes?: PieceJointeUpdateManyWithoutCategoriesInput;

  @ApiProperty({
    required: false,
    type: String,
  })
  @IsString()
  @MaxLength(1000)
  @IsOptional()
  @Field(() => String, {
    nullable: true,
  })
  piecesJointes?: string | null;
}

export { CategorieUpdateInput as CategorieUpdateInput };
