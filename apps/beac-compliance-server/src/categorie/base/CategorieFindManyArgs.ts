/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { CategorieWhereInput } from "./CategorieWhereInput";
import { IsOptional, ValidateNested, IsInt } from "class-validator";
import { Type } from "class-transformer";
import { CategorieOrderByInput } from "./CategorieOrderByInput";

@ArgsType()
class CategorieFindManyArgs {
  @ApiProperty({
    required: false,
    type: () => CategorieWhereInput,
  })
  @IsOptional()
  @ValidateNested()
  @Field(() => CategorieWhereInput, { nullable: true })
  @Type(() => CategorieWhereInput)
  where?: CategorieWhereInput;

  @ApiProperty({
    required: false,
    type: [CategorieOrderByInput],
  })
  @IsOptional()
  @ValidateNested({ each: true })
  @Field(() => [CategorieOrderByInput], { nullable: true })
  @Type(() => CategorieOrderByInput)
  orderBy?: Array<CategorieOrderByInput>;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  skip?: number;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  take?: number;
}

export { CategorieFindManyArgs as CategorieFindManyArgs };
